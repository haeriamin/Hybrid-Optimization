...
# const_nm is a small python package allowing to run a Nelder-Mead optimization via scipy's fmin function.
...
def constrNM(func, x0, LB, UB, args=(), xtol=0.0001, ftol=0.0001, maxiter=None, maxfun=None, full_output=0, disp=0, retall=0, callback=None, save=True):
    ...
    # Call fmin (Nelder-Mead)
    # removed
    # res=sciopt.fmin(constrObjFunc,x0,args=opts,ftol=ftol,xtol=xtol,maxiter=maxiter,disp=disp,
    # added
    res=fmin(constrObjFunc, x0, args=opts, ftol=ftol, xtol=xtol, maxiter=maxiter, disp=disp, full_output=full_output, callback=callback, maxfun=maxfun, retall=retall)
    ...
    # added
    if save:
        with open('last_solution.pkl', 'w') as f:
            pickle.dump(r, f)
